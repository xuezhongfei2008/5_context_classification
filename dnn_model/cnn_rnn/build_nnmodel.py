...
class TRNNConfig(object):
    ...
    def __init__(self):
        ...
        # RNN 模型构建参数
        # self.num_layers = 1  # RNN 层数
        # self.hidden_dim = 100  # RNN神经元个数

        self.hidden_dim_num = [100]  # RNN神经元个数

        self.layer_dimension = [50]  # 全连接层神经元,中间层
...
class TextRNN(object):
    ...
    def cnn_conv_pool(self):
        embedding_inputs = self.input_embedding()
        print("embedding_inputs_cnn", embedding_inputs.shape)
        pooled_outputs = []
        for filter_size in self.config.filter_sizes:
            with tf.name_scope("cnn%s" % filter_size):
                filter_shape = [filter_size, self.config.embedding_dim, 1, self.config.filter_num]
        ...
        self.h_pool = tf.reshape(pooled_outputs, [-1, len(self.config.filter_sizes), self.config.filter_num])
    ...
    def rnn(self):
        """CNN模型"""
        self.h_pool = self.cnn_conv_pool()
        print("CNN_h_pool", self.h_pool.shape)
        _outputs, _ = self.build_rnn_model()
        print("RNN_outputs", _outputs.shape)
        self.last = _outputs[:, -1, :]
        print("RNN_last", self.last.shape)
        # 计算损失函数
