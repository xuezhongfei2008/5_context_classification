...
def bulid_embedding():
    ...
    file_vocab = open("../data/vocab_word2vec.txt", 'w', encoding='utf-8')
    embedding_list = []
    ...
    # 保存词向量文件
    np.save("../data/embedding_word2vec.npy", embedding)
    file_vocab.close()
...
def build_traindata():
    ...
    df=pd.read_table("../data/train_keyword.txt",names=['calss','question'],sep='\t')
    print("df",df.head(1))
    categories=[]
    ...
    pickle_file = open('../data/categories.pkl', 'wb')
