...
train_dir = os.path.join(base_dir, 'train_keyword_500.txt')
test_dir = os.path.join(base_dir, 'test_keyword.txt')
val_dir = os.path.join(base_dir, 'test_keyword.txt')
...
class Run(object):
    ...
    def evaluate(self,model,sess, x_, y_):
        """评估在某一数据上的准确率和损失"""
        data_len = len(x_)
        batch_eval = batch_iter(x_, y_, 32)
        total_loss = 0.0
        total_acc = 0.0
            ...
            with session.as_default():
                ...
                for epoch in range(config.num_epochs):
                    ...
                    for x_batch, y_batch in batch_train:
                        ...
                        #每次迭代喂入得数据
                        feed_dict = self.feed_data(model,x_batch, y_batch, config.dropout_keep_prob)
                        ...
                        if total_batch % config.print_per_batch == 0:
                            # 每多少轮次输出在训练集和验证集上的性能
                            loss_train, acc_train, h_pool_flat= session.run([model.loss, model.acc,model.h_pool_flat], feed_dict=feed_dict)
                            loss_val, acc_val = self.evaluate(model,session, x_val, y_val)   # todo
                            # print("h_pool_flat",h_pool_flat)
                            # ff_pool.write(h_pool_flat+'\t')
                            # print("h_pool_flat",h_pool_flat)
    ...
    def get_poolout(self):
        ...
        x_test, y_test,contents,labels= process_file_poolout(test_dir, word_to_id, cat_to_id, config.seq_length)
        # print("x_test",x_test)
        # print("y_test",y_test)
        session = tf.Session()
        session.run(tf.global_variables_initializer())
        ...
        for i in range(num_batch):   # 逐批次处理
            ...
            start_id = i * batch_size
            end_id = min((i + 1) * batch_size, data_len)
            train_content=contents[start_id:end_id]
            train_label=labels[start_id:end_id]
            feed_dict = {
                model.input_x: x_test[start_id:end_id],
                model.keep_prob: 1.0
            }
            h_pool_flat= session.run(model.h_pool_flat, feed_dict=feed_dict)
            # print("x",x_test[start_id:end_id][0])
            # print("y", y_test[start_id:end_id][0])
            # print("h_pool_flat", h_pool_flat[0])
            # print("train_content", np.array(train_content[0]))
            # print("train_label", np.array(train_label))
            # print("aa",(x_test[start_id:end_id][0], y_test[start_id:end_id][0],h_pool_flat[0],))
            # c = np.vstack((x_test[start_id:end_id][0], y_test[start_id:end_id][0],h_pool_flat[0]))

            ff_pool.write(str(x_test[start_id:end_id][0]).replace("\n"," ")+"\t"+str(y_test[start_id:end_id][0]).replace("\n"," ")+\
                  "\t"+str(h_pool_flat[0]).replace("\n"," ")+"\t"+str(np.array(train_content[0])).replace("\n"," ")+"\t"\
                          +str(np.array(train_label)).replace("\n"," ")+"\n")
